...
for sIDX = 1:(numel(sList))
    ...
    Ons.raw{1} = behav.data.Voc(:,'onset');
    
    % get trial indices to assign ambiguity as parametric
    % modulator below
    Tr{1} = behav.data.Voc(:,'morphrate1');
  ...
  %Voice
    ...
    condition{1}.sound = condition_Voc
    ...
    Ons.run{1}.sound{1} = Ons.raw{1}(idXins_1,:)
    
    idXvoc_1 = [behav.data.Voc.morphrate1] == 0
    Ons.run{1}.sound{2} = Ons.raw{1}(idXvoc_1,:)
    
    idXins_08 = [behav.data.Voc.morphrate1] == 0.8
    Ons.run{1}.sound{3} = Ons.raw{1}(idXins_08,:)
    
    idXvoc_08 = [behav.data.Voc.morphrate1] == 0.2
    Ons.run{1}.sound{4} = Ons.raw{1}(idXvoc_08,:)
    
    idXins_06 = [behav.data.Voc.morphrate1] == 0.6
    Ons.run{1}.sound{5} = Ons.raw{1}(idXins_06,:)
    
    idXvoc_06 = [behav.data.Voc.morphrate1] == 0.4
    Ons.run{1}.sound{6} = Ons.raw{1}(idXvoc_06,:)
