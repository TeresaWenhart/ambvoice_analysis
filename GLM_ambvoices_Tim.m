...
for sIDX = 1:(numel(sList))
    ...
    Ons.raw{1} = behav.data.Tim(:,'onset');
    
    % get trial indices to assign ambiguity as parametric
    % modulator below
    Tr{1} = behav.data.Tim(:,'morphrate1');
    ...
    condition{1}.sound = condition_Tim
    ...
    Ons.run{1}.sound{1} = Ons.raw{1}(idXvlc_1,:)
    
    idXcla_1 = [behav.data.Tim.morphrate1] == 1
    Ons.run{1}.sound{2} = Ons.raw{1}(idXcla_1,:)
    
    idXvlc_08 = [behav.data.Tim.morphrate1] == 0.2
    Ons.run{1}.sound{3} = Ons.raw{1}(idXvlc_08,:)
    
    idXcla_08 = [behav.data.Tim.morphrate1] == 0.8
    Ons.run{1}.sound{4} = Ons.raw{1}(idXcla_08,:)
    
    idXvlc_06 = [behav.data.Tim.morphrate1] == 0.4
    Ons.run{1}.sound{5} = Ons.raw{1}(idXvlc_06,:)
    
    idXcla_06 = [behav.data.Tim.morphrate1] == 0.6
    Ons.run{1}.sound{6} = Ons.raw{1}(idXcla_06,:)
